...
- 稳定排序：对于相等的元素，排序后他们的相对位置不变，换句话说：如果在排序前 a[i] = a[j] 且 a[i] 在 a[j] 前面，那么排序后 a[i] 仍然在 a[j] 前面
  - 常见的稳定排序：冒泡、插入、归并、计数、基数
- 不稳定排序：对于相等的元素，排序后它们的相对位置可能发生改变。也就是说，在排序前 a[i] = a[j] 且 a[i] 在 a[j] 前面，排序后 a[i] 可能会出现在 a[j] 的后面
  - 选择、快速、希尔、堆
